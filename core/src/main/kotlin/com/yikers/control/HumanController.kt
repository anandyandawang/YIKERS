...
// instead, replicating YIKES exactly: vx = -tiltScale * accelX (raw, no
// deadzone, no clamp). Desktop has no accelerometer so it keeps the keys.
class HumanController(private val keys: KeyProfile = KeyProfile.ARROWS) : Controller {
    override fun decide(ctx: ControlContext): Move {
        ...
        val vx = if (tilt) {
            -ctx.tiltScale * Gdx.input.accelerometerX
        } else {
