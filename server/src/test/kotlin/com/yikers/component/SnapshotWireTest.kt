...
// (with its slot, augments and pending offer), a PropSnap stays a PropSnap (with its id).
class SnapshotWireTest {

    @Test
    fun sealedEntitySnapRoundTripsOverCbor() {
        val player = PlayerSnap(
            ShapeKind.CIRCLE, 1f, 0f, 0f, 1f, 2f, 3f, 0.5f, 0.5f, 0f, slot = 3,
            augments = listOf(AugmentId.DOUBLE_JUMP),
            offer = listOf(AugmentId.SWIFT_BOOTS, AugmentId.MOON_BOOTS, AugmentId.ADRENALINE),
        )
        ...
        assertEquals(2f, p.x)
        assertEquals(listOf(AugmentId.DOUBLE_JUMP), p.augments)
        assertEquals(listOf(AugmentId.SWIFT_BOOTS, AugmentId.MOON_BOOTS, AugmentId.ADRENALINE), p.offer)
